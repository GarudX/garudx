      ...
      {orientation === 'vertical' && (
        <div 
          className={cn(
            ...
            variant === 'standard' ? 'bg-titaniumSilver/20' : 'bg-electricBlue/20'
          )}
          aria-hidden="true"
        />
      )}
  ...
  const statusColors = {
    complete: {
      dot: variant === 'standard' 
        ? 'bg-signalGreen border-signalGreen text-white' 
        : 'bg-deepIndigo border-electricBlue/70 text-electricBlue',
    ...
    current: {
      dot: variant === 'standard'
        ? 'bg-electricBlue border-electricBlue text-white shadow-glow-blue'
        : 'bg-deepIndigo border-electricBlue text-electricBlue shadow-glow-blue',
      title: variant === 'standard'
        ? 'text-white'
        : 'text-electricBlue',
      content: variant === 'standard'
    ...
    upcoming: {
      dot: variant === 'standard'
        ? 'bg-darkSlate border-titaniumSilver/30 text-titaniumSilver/50'
        : 'bg-deepIndigo border-titaniumSilver/20 text-titaniumSilver/30',
      title: variant === 'standard'
        ? 'text-titaniumSilver/70'
        : 'text-titaniumSilver/50',
      content: variant === 'standard'
        ? 'text-titaniumSilver/50'
        : 'text-titaniumSilver/30',
    },
  };
      ...
      <div className={cn(
        ...
        >
          ...
          {variant === 'technical' && status === 'current' && (
            <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-electricBlue/50" />
          )}
        </Card>
      </div>
    );
      ...
      <div className={cn(
        ...
        >
          ...
          {variant === 'technical' && status === 'current' && (
            <>
              <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-electricBlue/50" />
              <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-electricBlue/50" />
            </>
          )}
        </Card>
      </div>
