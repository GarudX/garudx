...
// Standard Tailwind colors for charts
...
export const BarChart: React.FC<BaseChartProps> = ({
  ...
  return (
    <div 
      className={cn(
        'p-4 rounded-lg',
        variant === 'standard' ? 'bg-gray-800/70' : 'bg-gray-900/80 border border-gray-600/20',
        className
      )}
      style={{ width }}
    >
      ...
      <div 
        className={cn(
          'relative',
          variant === 'technical' && 'border border-gray-600/20'
        )}
        style={{ height }}
      >
        {/* Grid lines */}
            ...
            {[0, 0.25, 0.5, 0.75, 1].map((tick) => (
              <div 
                ...
                className={cn(
                  ...
                  variant === 'standard' ? 'border-gray-600/20' : 'border-blue-500/20',
                  'z-0'
                )}
                style={{ bottom: `${tick * 100}%` }}
              >
                {variant === 'technical' && (
                  <span className="absolute -top-2 -left-7 text-gray-400/50 text-xs font-mono">
        ...
        {variant === 'technical' && (
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_1px_1px,rgba(59,130,246,0.3)_1px,transparent_0)] bg-[length:20px_20px] opacity-10" aria-hidden="true" />
        )}
              ...
              >
                ...
                <div 
                  className={cn(
                    ...
                    variant === 'technical' && 'rounded-none border-t-2 border-blue-500/30'
                  )}
                  style={{ 
                    height: barHeight, 
                ...
                >
                  ...
                  {showValues && (
                    <div className={cn(
                      ...
                      variant === 'technical' && 'font-mono text-blue-400'
                    )}>
                      {item.value}
                    </div>
                  )}
                ...
                {showLabels && (
                  <div className={cn(
                    'text-gray-300 text-xs mt-2 truncate max-w-full',
      ...
      {showLegend && (
        <div className={cn(
          ...
          variant === 'technical' && 'border-t border-gray-600/20 pt-2'
        )}>
          {data.map((item, index) => (
      ...
      >
        ...
        >
          ...
          <polyline
            points={points}
            fill="none"
            stroke={variant === 'standard' ? '#3B82F6' : '#3B82F6'}
            ...
            return (
              <g key={index}>
                <circle
                  ...
                  cy={y}
                  r={4}
                  fill={variant === 'standard' ? '#3B82F6' : '#1F2937'}
                  stroke={variant === 'standard' ? 'white' : '#3B82F6'}
                  strokeWidth="1.5"
                ...
                {showValues && (
                  <text
                    ...
                    y={y - 10}
                    textAnchor="middle"
                    fill={variant === 'standard' ? 'white' : '#3B82F6'}
                    fontSize="10"
          ...
          <div className="absolute bottom-0 left-0 right-0 flex justify-between px-2">
            {data.map((item, index) => (
              <div 
                key={index}
                className={cn(
                  'text-gray-300 text-xs absolute -bottom-6',
                  variant === 'technical' && 'font-mono text-xxs tracking-wide'
                )}
  ...
  return (
    ...
    >
      ...
      <svg 
        ...
        className={cn(
          'relative z-10',
          variant === 'technical' && 'border border-gray-600/20 rounded-full'
        )}
      >
        {/* Segments */}
        {segments.map((segment, index) => (
          <path
            ...
            d={segment.path}
            fill={segment.color}
            stroke={variant === 'standard' ? 'rgba(0,0,0,0.1)' : 'rgba(59,130,246,0.2)'}
